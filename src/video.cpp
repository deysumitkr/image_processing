...
cv::Rect colorObjectDetectorAndDraw(cv::Mat &im) { // <-- Changed to non-const reference
    ...
    for (int i = 0; i < im.rows; i++) {
        for (int j = 0; j < im.cols; j++) {
            ...
            // Check the condition
            if (red > red_threshold_high && blue < blue_threshold_low && green < green_threshold_low) {
...
int main(int, char**)
{
    ...
    VideoCapture cap("../data/redCoke.mp4");
    ...
    cap.release(); // This will close the video file or camera
    // --- Destroy all windows ---
    destroyAllWindows(); // This will close all OpenCV windows
